...
- **口播稿必须过一遍用户的写作风格处理，而且过两遍（强制，别省）。** 先按解析梯子定风格源，再用它跑 —— ① 装了写作风格 skill（`personal-chinese-writing-style` / `personal-writing-style` / 任何风格类 skill）→ **用 Skill 工具调用它**，别手写风格清洗；② 没 skill 但 memory 里有写作风格记录 → 按它处理；③ 都没有 → 用下面第一 / 二遍列的内置基线规则兜底，并告诉用户「没找到写作风格 skill / memory，这遍用了内置基线」。一遍不够 —— 单遍通常只清掉标点和最扎眼的词，留下声音层的问题（最常见的是把工具 / agent 拟人成「人在打零工」的口语动词：默默干、喊一声、活儿、长在、丢进……）。无论用哪个风格源，两遍的分工都是：
...
- **停顿 / 呼吸节奏（长稿建议）。** 内容长了别从头讲到尾，在 `narration.txt` 的结构关节处标停顿 —— **空行** = 段落停顿（0.8s），**`[停 X]`** = 显式 X 秒（关键数字后 0.5、转折前 0.4）。标记会被剥掉再送 TTS、在音频里插真实静音、SRT 顺延，视频呼吸空间零漂移跟上。细节 / 推荐时长见 `listenhub-tts` 的「Step 0.7 · 停顿节奏」。
...
- [ ] 口播稿过了写作风格处理 **两遍**（标点一遍 + 声音一遍，见 Step 0；风格源按 Step 0 的解析梯子：写作风格 skill → memory → 内置基线，有 skill 就用 Skill 工具调用）
